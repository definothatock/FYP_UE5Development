...
void UInv_InventoryGrid::NativeOnInitialized()
{
	...
	HotbarComponent = UInv_InventoryStatics::GetHotbarComponent(GetOwningPlayer());
	HotbarComponent->OnHotbarUpdated.AddDynamic(this, &ThisClass::OnHotbarSlotUpdated);
}
...
void UInv_InventoryGrid::OnTileParametersUpdated(const FInv_TileParameters& Parameters)
{
	...
	{
	const FInv_GridFragment* GridFragment = GetFragment<FInv_GridFragment>(CurrentQueryResult.ValidItem.Get(), FGameplayTag::RequestGameplayTag(TEXT("FragmentTags.GridFragment"), /* ErrorIfNotFound */ true));
...
{
	const int32 UpperLeft = ResolveUpperLeftIndex(GridIndex);
	AssignHoverItem(ClickedInventoryItem, UpperLeft, UpperLeft);
	RemoveItemFromGrid(ClickedInventoryItem, UpperLeft);
}

int32 UInv_InventoryGrid::ResolveUpperLeftIndex(int32 GridIndex) const
{
	if (!GridSlots.IsValidIndex(GridIndex))
	{
		return GridIndex;
	}
	const int32 UL = GridSlots[GridIndex]->GetUpperLeftIndex();
	return UL != INDEX_NONE ? UL : GridIndex;
}
	...
	const int32 PreviousGridIndex)
{
	...
	HoverItem->UpdateHotbarIndex(GridSlots[GridIndex]->GetHotbarIndex());
}

void UInv_InventoryGrid::RemoveItemFromGrid(UInv_InventoryItem* InventoryItem, const int32 GridIndex)
{
	const FInv_GridFragment* GridFragment = GetFragment<FInv_GridFragment>(InventoryItem, FGameplayTag::RequestGameplayTag(TEXT("FragmentTags.GridFragment"), /* ErrorIfNotFound */ true));
	if (!GridFragment) return;
...
void UInv_InventoryGrid::AssignHoverItem(UInv_InventoryItem* InventoryItem)
{
	...
	const FInv_GridFragment* GridFragment = GetFragment<FInv_GridFragment>(InventoryItem, FGameplayTag::RequestGameplayTag(TEXT("FragmentTags.GridFragment"), /* ErrorIfNotFound */ true));
	const FInv_ImageFragment* ImageFragment = GetFragment<FInv_ImageFragment>(InventoryItem, FGameplayTag::RequestGameplayTag(TEXT("FragmentTags.IconFragment"), /* ErrorIfNotFound */ true));
...
void UInv_InventoryGrid::CreateItemPopUp(const int32 GridIndex)
{
	...
	if (!IsValid(ItemPopUp)) return;	// add a check and works, how come
	...
	if (RightClickedItem->IsEquippable())
	{
		...
		ItemPopUp->OnLogToHotbar.BindDynamic(this, &ThisClass::OnPopUpMenuLogToHotbar);
	}
	else
	{
		ItemPopUp->CollapseEquipRelatedButtons();
...
UInv_SlottedItem* UInv_InventoryGrid::FindSlottedItemByItem(UInv_InventoryItem* Item) const
{
...
void UInv_InventoryGrid::AddItem(UInv_InventoryItem* Item)
{
	...
	// Avoid duplicate widgets when OnItemAdded fires more than once for the same replicated item.
	if (FindSlottedItemByItem(Item))
	{
		return;
	}
...
void UInv_InventoryGrid::OnItemRemoved(UInv_InventoryItem* Item)
{
	...
	if (UInv_SlottedItem* SlottedItem = FindSlottedItemByItem(Item))
	{
		RemoveItemFromGrid(Item, SlottedItem->GetGridIndex());
	}
}
...
{
	const FInv_GridFragment* GridFragment = GetFragment<FInv_GridFragment>(Item, FGameplayTag::RequestGameplayTag(TEXT("FragmentTags.GridFragment"), /* ErrorIfNotFound */ true));
	const FInv_ImageFragment* ImageFragment = GetFragment<FInv_ImageFragment>(Item, FGameplayTag::RequestGameplayTag(TEXT("FragmentTags.IconFragment"), /* ErrorIfNotFound */ true));
	if (!GridFragment || !ImageFragment) return;
...
void UInv_InventoryGrid::UpdateGridSlots(UInv_InventoryItem* NewItem, const int32 Index, bool bStackableItem, const int32 StackAmount)
{
	...
	const FInv_GridFragment* GridFragment = GetFragment<FInv_GridFragment>(NewItem, FGameplayTag::RequestGameplayTag(TEXT("FragmentTags.GridFragment"), /* ErrorIfNotFound */ true));
...
void UInv_InventoryGrid::ClearHoverItem()
{
	...
	HoverItem->UpdateHotbarIndex(INDEX_NONE);	/////////////
...
void UInv_InventoryGrid::SwapWithHoverItem(UInv_InventoryItem* ClickedInventoryItem, const int32 GridIndex)
{
	...
	const int32 ClickedUpperLeft = ResolveUpperLeftIndex(GridIndex);
	// Keep the same previous grid index
	AssignHoverItem(ClickedInventoryItem, ClickedUpperLeft, PreviousGridIndex);
	RemoveItemFromGrid(ClickedInventoryItem, ClickedUpperLeft);
...
void UInv_InventoryGrid::OnPopUpMenuEquip(int32 Index)
{
	...
	FGameplayTag EquipmentTypeTag = UpperLeftGridSlot->GetInventoryItem()->GetItemManifest().GetItemType();
	RequestEquipItem_SlotClicked.Broadcast(UpperLeftGridSlot, EquipmentTypeTag);
}
...
void UInv_InventoryGrid::OnPopUpMenuConsume(int32 Index)
{
	...
	if (NewStackCount <= 0)
	{
		RemoveItemFromGrid(RightClickedItem, ResolveUpperLeftIndex(Index));
	}
}

void UInv_InventoryGrid::OnPopUpMenuLogToHotbar(int32 HotbarIndex, int32 Index)
{
	UInv_InventoryItem* RightClickedItem = GridSlots[Index]->GetInventoryItem().Get();
	if (!IsValid(RightClickedItem)) return;
	...
	FGameplayTag EquipmentTypeTag = RightClickedItem->GetItemManifest().GetItemType();
	if (RightClickedItem->IsEquippable())
	{
		TryAddHotbarSlot.Broadcast(RightClickedItem, HotbarIndex);
	}
}
...
void UInv_InventoryGrid::OnHotbarSlotUpdated(int32 HotbarIndex, UInv_InventoryItem* Item)
{
	if (UInv_SlottedItem* Slotted = FindSlottedItemByItem(Item))
	{
		Slotted->UpdateHotbarIndex(HotbarIndex); // -1 Collapsed, others assign and HitTestInvisible
	}
	// GridSlots[
}
