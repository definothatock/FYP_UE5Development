...
void UInv_SlottedItem::UpdateHotbarIndex(int32 Index)
{
	HotbarIndex = Index;
	if (Index >= 0)
	{
		Text_HotbarIndex->SetVisibility(ESlateVisibility::HitTestInvisible);
		Text_HotbarIndex->SetText(FText::AsNumber(Index+1));
	}
	else
	{
		Text_HotbarIndex->SetVisibility(ESlateVisibility::Collapsed);
	}
}

void UInv_SlottedItem::UpdateEquippedBorder(bool bEquipped) const
{
	if (bEquipped)
	{
