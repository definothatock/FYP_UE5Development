...
UInv_InventoryItem* FInv_InventoryFastArray::AddEntryFromManifest(const FInv_ItemManifest& InManifest)
{
	check(OwnerComponent);
	AActor* OwningActor = OwnerComponent->GetOwner();
	...
	FInv_ItemManifest Copy = InManifest;
	UInv_InventoryItem* NewItem = Copy.Manifest(OwningActor);
	if (!IsValid(NewItem))
	{
		return nullptr;
	}
	...
	int32 StackCount = 1;
	if (const FInv_StackableFragment* SF = NewItem->GetItemManifest().GetFragmentOfType<FInv_StackableFragment>())
	{
		StackCount = FMath::Max(1, SF->GetStackCount());
	}
	NewItem->SetTotalStackCount(StackCount);

	FInv_InventoryEntry& NewEntry = Entries.AddDefaulted_GetRef();
	NewEntry.Item = NewItem;
	IC->AddRepSubObj(NewItem);
	MarkItemDirty(NewEntry);
