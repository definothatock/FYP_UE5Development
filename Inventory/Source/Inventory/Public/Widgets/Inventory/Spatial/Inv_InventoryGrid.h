...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FRequestEquipItem_SlotClicked, UInv_GridSlot*, GridSlot,
                                             const FGameplayTag&, EquipmentTypeTag);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FTryAddHotbarSlot,UInv_InventoryItem*, Item, int32, HotbarIndex);
...
public:
	...
	FRequestEquipItem_SlotClicked RequestEquipItem_SlotClicked;

	UPROPERTY(BlueprintAssignable)
	FTryAddHotbarSlot TryAddHotbarSlot;
...
private:
	...
	/** Upper-left index for an item footprint (any cell in the footprint is valid input). */
	int32 ResolveUpperLeftIndex(int32 GridIndex) const;
	...
	UInv_SlottedItem* FindSlottedItemByItem(UInv_InventoryItem* Item) const;
	...
	UFUNCTION()
	void OnPopUpMenuLogToHotbar(int32 HotbarIndex, int32 Index);
	
	UFUNCTION()
	void OnInventoryMenuToggled(bool bOpen);

	UFUNCTION()
	void OnHotbarSlotUpdated(int32 HotbarIndex, UInv_InventoryItem* Item);
