...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Inventory")
	static UInv_HotbarComponent* GetHotbarComponent(const APlayerController* PlayerController);
	...
	/** Sum of every player's GetAccumulatedClearedScore in the world. Server / authority only. */
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category = "Inventory|Score", meta = (WorldContext = "WorldContextObject"))
	static int32 GetTeamAccumulatedClearedScore(UObject* WorldContextObject);
